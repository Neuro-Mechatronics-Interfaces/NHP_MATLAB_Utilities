function outPath = concat_files(inFile, exts, options)
%CONCAT_FILES Concatenate the text of a target Javascript file and
% other files from specified folders/extensions into a single output file.
%
% outPath = utils.concat_files(inFile, Name=Value, ...)
%
% Required
...
%   exts                 string    =               % which file extensions to include
%
% Name-Value options (parsed into `options` via arguments block):
%   OutputFile                 string    = "_%s_.concatenated"
...
arguments
    inFile (1,1) string
    exts (1,:) string = "";
    options.OutputFile (1,1) string = "_%s_.concatenated"
...
if ~isfile(inFull)
    error('concat_file_contents:InputNotFound', ...
        'Input file not found: %s', inFull);
end

% ---- Normalize extensions (ensure they start with ".") ----
if nargin < 2 && ~isempty(inExt) && isscalar(exts) && strlength(exts(1))<1
    exts = string(inExt);
end
...
if contains(outName,"%s")
    matches = regexp(outName, "%s", 'match');
    outName = sprintf(outName,exts(1:min(length(matches),numel(exts)))); 
end
...
% =====================================================================
    ...
    function e = ensureDotPrefix(eIn)
        e = string(eIn);
        if strlength(e) < 1
            error("All requested extensions must have at least one character!"); 
        end
        if ~startsWith(e, ".")
            e = sprintf('.%s',e);
        end
