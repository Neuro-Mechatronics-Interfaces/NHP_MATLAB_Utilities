...
opts.TMSi.Array.Apply_HPF = false;                          % Apply high-pass filter to the data?
opts.TMSi.Array.Apply_Max_Rescale = true;                   % Apply rescaling using maximum value on per-channel basis?
opts.TMSi.Array.Apply_Polynomial_Detrend = true;            % Apply polynomial detrending to the data?
...
opts.TMSi.Array.HPF_Order = 4;                              % Butterworth filter order for High-pass filter
opts.TMSi.Array.HPF_Cutoff_Frequency = 100;                 % Cutoff frequency for High-pass filter
opts.TMSi.Array.Polynomial_Detrend_Order = 7;               % If this is NaN, auto-compute polynomial detrend order from number of triggers. Otherwise, use a specified polynomial fit to detrend data.
